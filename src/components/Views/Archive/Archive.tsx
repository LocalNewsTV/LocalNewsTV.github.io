...
const ProjectTable = () => {
  ...
  const filterProjects = () => {
    if (filter) {
      return projectsList.filter((entry: { [key: string]: any }) =>
        Object.keys(entry).some((key: string) =>
          JSON.stringify(entry[key]).toLowerCase().includes(filter.toLowerCase()),
        ),
      );
    }
    return projectsList;
  ...
  return (
    <>
      <input type="text" onChange={changeHandler} value={filter} />
      <StyledTable>
        <thead>
          <StyledRow>
  ...
  return (
    <ViewContainer>
      <ContentContainer>
        ...
        <SubHeader>A big list of things I've worked on</SubHeader>
        <ProjectTable />
      </ContentContainer>
    </ViewContainer>
  );
